...
class RolloutStorage(object):
    def __init__(self, num_steps, mini_batch_num, obs_shape,owner='s'):
        ...
        self.action = torch.zeros(num_steps, 1)
    ...
    def compute_returns(self, next_value, use_gae, gamma,tau):
        ...
        #         debug1 = self.value_preds[step + 1]
        #         debug2 = self.value_preds[step]
        #         delta = self.rewards[step] + gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
        #         gae = delta + gamma * tau * self.masks[step + 1] * gae
        #         debug = gae + self.value_preds[step]
        #         self.returns[step] = gae + self.value_preds[step]
        # else:
        self.returns[-1] = next_value
        for step in reversed(range(self.rewards.size(0))):
            self.returns[step] = self.returns[step + 1] * gamma  + self.rewards[step]
        debug = self.returns[-1]
        stop = True
    ...
    def feed_forward_generator(self, advantages):
        ...
        debug = SubsetRandomSampler(range(self.num_steps))
