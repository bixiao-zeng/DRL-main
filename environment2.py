...
class Env(object):
    def __init__(self,args, device, env_args,
        ...
        #========PPO-server===============
        self.args = args
        self.local_ppo_model = Model(self.args.state_dim_s, self.args.action_dim_s, device,owner='s',num_user=self.args.user_num)
        self.optimizer = optim.Adam(list(self.local_ppo_model.parameters()), lr=self.args.lr_s)
        self.rollout = RolloutStorage(self.args.exploration_steps, self.args.mini_batch_num, self.args.state_dim_s,owner='s')
        ...
        self.datasize = env_args.V['datasize']
        #=================================
        self.device = device
        ...
        unit_cost = env_args.V['cost'][:self.args.user_num]
        self.unit_cost = unit_cost
        self.uni_quality = env_args.V['uni_quality'][:self.args.user_num]
        # self.uni_quality = np.exp(self.uni_quality)/np.sum(np.exp(self.uni_quality))
        self.A = sum(np.array(unit_cost) / np.array(self.uni_quality))
        self.nash_useraction = np.zeros(self.args.user_num)
        self.nash_userreward = np.zeros(self.args.user_num)
        self.nash_reward = 0
        ...
        # self.check_cost()
        cost_max = max(unit_cost)
        lft = self.A
        rgt = 2*(self.user_num-1)*np.min(self.unit_cost)
    ...
    def constrain_cmp2(self):
        ...
        constrain_grad = torch.randn(3,self.user_num)
        ...
        constrain = torch.randn(3,self.user_num)
    ...
    def nash_optimization(self,lr=1e-3):
        ...
        self.nash_useraction = Variable(torch.randn(self.user_num), requires_grad=True)
        self.multiplier = Variable(torch.randn(self.user_num,3),requires_grad=True)
        # self.constrain_grad = torch.randn(self.user_num,3)
        # self.constrain = torch.randn(self.user_num,3)
        ...
        #             milestones=[1700], gamma=0.1)
        # optim.lr_scheduler.StepLR(optimizer, 50, gamma=0.1, last_epoch=-1)
        L_grad = torch.randn(self.user_num)
        u_grad = torch.randn(self.user_num)
        PHI = torch.randn(self.user_num,3)
        self.nash_serverAct()
        num_epochs = 5000
        LOSS = []
        min_loss = 1e8
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                condi2 = torch.ge(u,0).all()
                condi3 = torch.ge(self.nash_useraction,1).all()
    ...
    def nash_computing(self):
        ...
        for i in range(self.user_num):
            ...
            op_lst.append(x*(1-x*self.unit_cost[i]/self.uni_quality[i]))
        self.nash_action = self.lamda*self.al  - 1 / (sum(op_lst)*self.bt)
        self.nash_action = int(self.clamp_number(self.nash_action, 0, self.remain_budget))
        for i in range(self.user_num):
            x = (self.user_num - 1) / (self.uni_quality[i] * self.A)
            self.nash_useraction[i] = x * self.nash_action*(1-x*self.unit_cost[i])*self.omega
            self.nash_useraction[i] = int(self.clip_action(i))
            sensing_data += self.uni_quality[i]*self.nash_useraction[i]
            ss_lst.append(self.uni_quality[i]*self.nash_useraction[i])
        #=================nash reward computing=======================
        # self.norm_action()
        quality_data = np.array(self.uni_quality)*self.nash_useraction
        debug1 = quality_data/np.sum(quality_data)
        debug2 = np.exp(quality_data)/np.sum(np.exp(quality_data))
        for i in range(self.user_num):
            share = self.uni_quality[i]*self.nash_useraction[i]/sensing_data
            self.nash_userreward[i] = share*self.nash_action - self.unit_cost[i]*self.nash_useraction[i]
        self.nash_reward = self.lamda*self.g_concav(sensing_data)-self.nash_action
        debug = True
    ...
    def update(self):
        ...
        return av_value_loss / loss_cnt, av_policy_loss / loss_cnt, av_ent_loss / loss_cnt, total_loss/loss_cnt
    ...
    def reset(self):
        ...
        state_s = np.zeros((1, self.state_dim_s))
        state_c = np.zeros((self.user_num, self.state_dim_c))
        self.remain_budget = self.args.remain_budget
        return torch.from_numpy(state_s).float().to(self.device),torch.from_numpy(state_c).float().to(self.device)
    ...
    def step_stage1(self, state_s,state_c):
        for i in range(self.args.user_num):
            state_c[i,-1] = self.R
        self.remain_budget -= self.R
        state_s[0,0] = self.R
        state_s[0,2] = self.remain_budget
    ...
    def step_stage2(self, action,state_s,state_c):
        ...
        # -------standard action----------------------
        # for i in range(self.user_num):
        #     if action[i] > self.datasize[i]:
        #         action[i] = self.datasize[i]
        ...
        quality_sensing_data = action*self.uni_quality

        self.total_quality_sensing_data = np.sum(quality_sensing_data)+1e-6

        reward = np.zeros(self.user_num)
        self.server_reward_cur = self.g_concav(self.total_quality_sensing_data) * self.lamda - self.R
        self.server_reward += self.server_reward_cur
        self.total_task += 1
        self.complete_task += 1
        print('==========env step=========')
        print('server bonus:%f'%(self.R))
        quality_sensing_nashdata = [
            self.nash_useraction[ix] * self.uni_quality[ix] for ix in
            range(self.user_num)]
        total_quality_sensing_nashdata = np.sum(quality_sensing_nashdata) + 1e-6
        for i in range(self.user_num):
            # reward[i] = action[i] / total_sensing * self.R - self.unit_cost[i] * action[i] + phi[i]
            debug_a = quality_sensing_nashdata[i]/total_quality_sensing_nashdata
            debug_b = quality_sensing_data[i] / self.total_quality_sensing_data
            reward[i] = quality_sensing_data[i] / self.total_quality_sensing_data * self.R  - self.unit_cost[i] * action[i]
            # reward[i] = 0 if reward[i]<0 else reward[i]
            print('user %d action: %f utility: %f' % (i, action[i], reward[i]))
        self.rewards = reward

        contribution = self.uni_quality * action
        for i in range(self.user_num):
            #==============no observing others===============
            # state_c[i, 0] = torch.tensor(action[i])
            # state_c[i, 1] = reward[i]
            # ==============observing others' strategies===============
            # contribution = self.uni_quality*action
            # state_c[i,0:self.user_num] = torch.tensor(contribution)
            # state_c[i,self.user_num] = reward[i]
            #==============observing others' income===============

            state_c[i, 0:self.user_num] = torch.tensor(contribution)
            state_c[i, self.user_num:self.user_num*2] = torch.tensor(reward)

            # self.remain_data[i] -= action[i]
            # state_c[i, -1] = self.remain_data[i]
        # reward = np.mean(reward, keepdims=True)

        state_s[0,1] = self.total_quality_sensing_data
        state_s[0,-2] = self.server_reward
        state_s[0,-1] = self.server_reward_cur
        done = False

        return state_s,state_c, torch.from_numpy(reward).float(), done